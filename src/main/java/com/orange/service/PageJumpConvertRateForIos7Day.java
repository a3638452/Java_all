...
public class PageJumpConvertRateForIos7Day implements Serializable{
	
public  void PageJumpConvertRateForIos7Day() {
	//1.构建sparksession
...
private  JavaRDD<Row> generateRDD(SparkSession spark) {
	 JavaRDD<String> pageRDD = spark.sparkContext().textFile("hdfs://master:9000/user/hadoop/bingbing/ios/pagedata7day.txt", 1).toJavaRDD();
...
PageSplitConvertRate pageSplitConvertRate2 = new PageSplitConvertRate();
	...
	//执行插入方法
	IosPageSplitConvertRateDAOImpl7Day pageSplitConvertRateDAO = daoFactory.getIosPageSplitConvertRateDAO07Day();
