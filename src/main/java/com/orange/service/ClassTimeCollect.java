	...
	public static void main(String[] args) {
		 ...
		 JavaRDD<String> hdfsRDD = spark.read().textFile("hdfs://master:9000/SDKData/total/data" + DateUtils.getYesterdayDate() + "/streamingdata" + DateUtils.getYesterdayDate() + ".txt").toJavaRDD();
		  ...
		  String schemaString = "user_id class_id carme_id start_time end_time use_time";
		    ...
		    Dataset<Row> resultDF1 = spark.sql("select  user_id,class_id,carme_id,start_time,end_time,use_time from streamingData");
		    
		    resultDF1.createOrReplaceTempView("distinctresult");            
		    Dataset<Row> resultDF2 = spark.sql("select class_id ,"+
		                           "sum(use_time) total_time,"+
		                           "from_unixtime(unix_timestamp()-86400,'yyyy-MM-dd ') report_time,"+
		                           "from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') create_time"+
		                           " from distinctresult group by class_id ");
		    resultDF2.write().mode("append").jdbc(Constants.URL_TEST, Constants.JDBC_LIVING_STREAMING_DURATION, JdbcUtil.JdbcCon());
		    }
		 spark.stop();
	}
}
